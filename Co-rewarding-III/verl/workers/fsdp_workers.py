...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def init_model(self):
        ...
        if self._is_ref:
            ...
            # 将 ref 模型注入到 rollout 的 sharding manager（仅当本实例也含有rollout时）
            if hasattr(self, "rollout_sharding_manager"):
                print("inject ref model to rollout sharding manager")
                self.rollout_sharding_manager.set_ref_module(self.ref_module_fsdp)
    ...
    def update_actor(self, data_ori: DataProto, data_aug: DataProto):
        ...
        with self.ulysses_sharding_manager:
            data_ori = data_ori.to("cpu")  # data will to device with each micro batch on actor.update_policy
            data_aug = data_aug.to("cpu")

            # perform training
            with Timer(name="update_policy", logger=None) as timer:
                metrics = self.actor.update_policy(data_ori=data_ori, data_aug=data_aug)
            delta_time = timer.last
            global_num_tokens = data_ori.meta_info["global_token_num"]
    ...
    @register(dispatch_mode=make_nd_compute_dataproto_dispatch_fn(mesh_name="rollout"))
    @DistProfiler.annotate(color="green", role="ref_rollout_generate")
    def generate_ref_sequences(self, prompts: DataProto):
        """Generate sequences using ref model, optionally updating ref model with sliding average.
        
        Args:
            prompts (DataProto): Input prompts for generation with sliding average parameters in meta_info
        """
        # to device
        prompts = prompts.to(get_device_id())

        assert self._is_rollout and hasattr(self, "rollout_sharding_manager")

        timing_generate = {}
        
        # 从meta_info中读取滑动平均参数
        update_ref_model = prompts.meta_info.get("update_ref_model", False)
        # alpha = prompts.meta_info.get("sliding_alpha", 0.0)
        alpha_start = prompts.meta_info.get("sliding_alpha_start", 0.3)
        alpha_end = prompts.meta_info.get("sliding_alpha_end", 0.05)
        total_steps = prompts.meta_info.get("total_steps", 500)
        current_step = prompts.meta_info['current_step']
        
        # 清理meta_info中的滑动平均参数，避免传递到后续处理
        prompts.meta_info.pop("update_ref_model", None)
        prompts.meta_info.pop("sliding_alpha", None)
        prompts.meta_info.pop("sliding_alpha_start", None)
        prompts.meta_info.pop("sliding_alpha_end", None)
        prompts.meta_info.pop("total_steps", None)
        prompts.meta_info.pop("current_step", None)

        # 如果需要更新ref model，设置更新标志
        if update_ref_model:
            self.rollout_sharding_manager.set_update_ref_model(
                update_ref_model=True,
                alpha_start=alpha_start,
                alpha_end=alpha_end,
                total_steps=total_steps
            )
            self.rollout_sharding_manager.update_alpha(step=current_step)
            print(f"Will update ref model with sliding average during this rollout (alpha={self.rollout_sharding_manager.sliding_alpha})")

        # 切换到ref权重
        self.rollout_sharding_manager.set_use_ref_module(True)
        try:
            with self.rollout_sharding_manager:
                log_gpu_memory_usage("After entering ref rollout sharding manager", logger=logger)

                with simple_timer("generate_ref_sequences", timing_generate):
                    output = self.rollout.generate_sequences(prompts=prompts)

                log_gpu_memory_usage("After ref rollout generation", logger=logger)
        finally:
            # 恢复为actor
            self.rollout_sharding_manager.set_use_ref_module(False)

        # 聚合timing并返回
        timing_generate.update(self.rollout_sharding_manager.timing)
        topk_ratio, tmin, tmax = topk_reduce_ratio_min_max(timing_generate["generate_ref_sequences"])
        timing_generate = reduce_timing(timing_generate)
        timing_generate.update({
            "ref_generation_timing/max": tmax,
            "ref_generation_timing/min": tmin,
            "ref_generation_timing/topk_ratio": topk_ratio,
        })
        output.meta_info["timing"] = timing_generate
        output = output.to("cpu")
        get_torch_device().empty_cache()
    ...
    # @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    # def update_ref_model_sliding_average(self, alpha: float = 0.0):
    #     """Update ref model with sliding average of policy model and ref model.
        
    #     Args:
    #         alpha (float): Weight for policy model (1-alpha for ref model)
    #     """
    ...
    #             alpha=alpha, 
    #         )
    #         print(f"Set ref model sliding average update flag (alpha={alpha})")
    #     else:
    #         print("Warning: rollout_sharding_manager not available for ref model update")
