...
@register("co_rewarding_II")
class CoRewardingII(AbstractRewardManager):
    """The reward manager."""
    ...
    def __call__(self, data: DataProto, return_dict: bool = False) -> torch.Tensor | dict[str, Any]:
        ...
        if "ref_responses" not in data.batch.keys():
            raise ValueError("ref_responses is not supported")

        #  1. 提取 data 的 pseudo label
        uid2answers = defaultdict(list)
        uid2policy_answers = defaultdict(list)
        all_policy_answers = []
        for i, item in enumerate(data):
            ...
            uid2policy_answers[uid].append(ans_policy)
            all_policy_answers.append(ans_policy)
        ...
        uid2policypseudo = {uid: majority_vote(ans_list, empty_value='') for uid, ans_list in uid2policy_answers.items()}
        ...
        gt_pseudo_correct = 0
        gt_empty = 0
        gt_policypseudo_correct, gt_refpseudo_correct = 0, 0
        ...
        for i in range(N):
            item = data[i]   
            ans = all_policy_answers[i]
            pseudo_label = uid2pseudo[item.non_tensor_batch["uid"]]
            pseudo_policy_label = uid2policypseudo[item.non_tensor_batch["uid"]]
            if pseudo_label != '':
                pseudo_nonempty += 1
            ...
            if gt is not None and str(gt).strip() != '':
                if _answers_equal(gt, pseudo_label):
                    gt_pseudo_correct += 1
                    gt_refpseudo_correct += 1
                if _answers_equal(gt, pseudo_policy_label):
                    gt_policypseudo_correct += 1
            else:
                gt_empty += 1
        ...
        pseudo_policy_acc = gt_policypseudo_correct / gt_total_pseudo
        pseudo_ref_acc = gt_refpseudo_correct / gt_total_pseudo
        ...
        metrics = {
            ...
            "coreward/pseudo_policy_acc": pseudo_policy_acc,
            "coreward/pseudo_ref_acc": pseudo_ref_acc,
