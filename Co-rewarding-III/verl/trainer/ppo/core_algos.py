...
class AdvantageEstimator(str, Enum):
    ...
    GPG = "gpg"
    CO_REWARDING_GRPO = "co_rewarding_grpo"
...
def agg_loss_per_sample(loss_mat: torch.Tensor,
                        loss_mask: torch.Tensor,
                        eps: float = 1e-8) -> torch.Tensor:
    """
    按样本保留 batch 维度，返回每个样本内部的 token-mean loss。
    
    Args:
        loss_mat: (bs, response_length)
        loss_mask: (bs, response_length)  1 表示有效 token, 0 表示无效
        eps: 防止除零
    
    Returns:
        loss_per_sample: (bs,) 每个样本的平均 loss
    """
    # 每个样本的有效 token 数: (bs,)
    denom = loss_mask.sum(dim=-1).clamp(min=eps)

    # 每个样本内部的总 loss: (bs,)
    numer = (loss_mat * loss_mask).sum(dim=-1)

    # token-mean loss per sample: (bs,)
    loss_per_sample = numer / denom
